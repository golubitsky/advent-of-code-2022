...
module Solution
  ...
  def solution(monkeys)
    ...
    20.times { perform_round(monkeys, monkey_inspection_counts) }
    level_of_monkey_business = monkey_inspection_counts.sort_by do |_index, count|
      -count
    end.to_h.values.take(2).reduce(:*)
  end

  private

  def perform_round(monkeys, monkey_inspection_counts)
    monkeys.each_with_index do |monkey, index|
      monkey_inspection_counts[index] += monkey[:starting_items].count

      multiplied = monkey[:starting_items].map { |item| apply_operation(item, monkey[:operation]) }
      bored = multiplied.map { |item| item / 3 }
